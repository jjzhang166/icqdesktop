...
namespace Ui
{
	...
	void LoginPage::init()
	{
		...
		QObject::connect(Ui::GetDispatcher(), SIGNAL(getSmsResult(int)), this, SLOT(getSmsResult(int)), Qt::DirectConnection);
	...
	{
		setFocus();
		prev_page_link_->setVisible(index == SUBPAGE_PHONE_CONF_INDEX);
		next_page_link_->setVisible(index != SUBPAGE_PHONE_CONF_INDEX);
		next_page_link_->setText(index == SUBPAGE_PHONE_LOGIN_INDEX ? QT_TRANSLATE_NOOP("login_page","Continue") : QT_TRANSLATE_NOOP("login_page","Start messaging"));
	...
	void LoginPage::nextPage()
	{
		...
		else if (login_staked_widget_->currentIndex() == SUBPAGE_UIN_LOGIN_INDEX)
		{
			...
			GetDispatcher()->post_message_to_core("login_by_password", collection.get());
		}
	}
	...
	void LoginPage::codeEditChanged(QString code)
	{
		if (code.length() == 4)
		{
			...
			GetDispatcher()->post_message_to_core("login_by_phone", collection.get());
		}
	}
