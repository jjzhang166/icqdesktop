...
int dbCompiler::scan()
{
   ...
   nextElement:
    ...
    do {
        ch = *p++;
        if (ch == '\n') {
            offsetWithinStatement = (int)((char_t*)queryElement->ptr - p);
            firstPos = 0;
        }
    } while (ch > 0 && ch <= ' ');
    currPos += (int)(p - q);
...
dbExprNode* dbCompiler::comparison()
{
        ...
        || cop == tkn_like || cop == tkn_match || cop == tkn_not || cop == tkn_is || cop == tkn_in)
    {
        ...
        if (cop == tkn_in) {
            ...
            expr->regex.opd = left;
            left = expr;
            if (regcomp(&expr->regex.re, (char*)right->svalue.str, REG_EXTENDED|REG_ICASE|REG_NOSUB) != 0) { 
...
{
    TRACE_MSG((STRLITERAL("Compile expression for table %s\n"), table->name));    
#ifdef DO_NOT_USE_SETJMP
    try 
#else
...
void dbTrace(char_t* message, ...) 
{
    if (dbTraceEnable) { 
        ...
        char_t buffer[1024];
        VSPRINTF(SPRINTF_BUFFER(buffer), message, args);
...
HANDLE g_hGigabaseHeap = NULL;

typedef stdext::hash_map<DWORD, DWORD> HASHDWORD;

HASHDWORD *g_Hash = NULL;
CRITICAL_SECTION *g_cs = NULL;


void dbCreateHeap()
{
	assert(!g_hGigabaseHeap);
	g_hGigabaseHeap = ::HeapCreate(0, 0, 0);
	assert(g_hGigabaseHeap);
...
byte* dbMalloc(size_t size)
{
	...
	if (!g_hGigabaseHeap)
		dbCreateHeap();
	if (!g_Hash)
		g_Hash = new HASHDWORD;
	if (g_hGigabaseHeap)
	{
		byte *p = (byte *)::HeapAlloc(g_hGigabaseHeap, 0, size);
		assert(p);
		EnterCriticalSection(g_cs);
		g_Hash->insert(HASHDWORD::value_type((DWORD)p, (DWORD)p));
...
void  dbFree(void* p)
{
	...
	if (g_hGigabaseHeap)
	{
#ifdef _DEBUG
		EnterCriticalSection(g_cs);
		HASHDWORD::iterator it = g_Hash->find((DWORD)p);
		if (it == g_Hash->end())
		{
			HASHDWORD Hash = *g_Hash;
			std::string s;
			for (it = g_Hash->begin(); it != g_Hash->end(); it++)
			{
				char str[1024];
				sprintf(str, "%.8X\r\n", (DWORD)it->first);
				s += str;				
			}
			int iFile = open("e:\\mem.log", O_CREAT | O_TRUNC | O_BINARY | O_RDWR, S_IREAD | S_IWRITE);
			write(iFile, (BYTE *)s.c_str(), s.length());
			close(iFile);
			int a = 0;
		}
		g_Hash->erase(it);
		...
		if (0 == ::HeapValidate(g_hGigabaseHeap, 0, p))
		{
			int iErr = ::GetLastError();
			iErr = iErr;
		}
#endif
		::HeapFree(g_hGigabaseHeap, 0, p);
		return;
	}	
#endif
    free(p);
