...
static int cli_close_internal(session_desc* s)
{
  ...
  public:
                      ...
                      char_t const* password)
    {
        ...
        for (session_desc* desc = connection_chain; desc != NULL; desc = desc->next_pooled) {
            ...
            {
                connection_chain = desc->next_pooled;
                return desc;
            }
        }
        return NULL;
    ...
    {
        dbCriticalSection cs(mutex);
        desc->next_pooled = connection_chain;
        connection_chain = desc;
    }

    void close() { 
        dbCriticalSection cs(mutex);
        session_desc *desc, *next;
        for (desc = connection_chain; desc != NULL; desc = next) {
            desc->pool = NULL;
            delete[] desc->user;
            delete[] desc->password;
            next = desc->next_pooled;
            cli_close_internal(desc);
        }
        connection_chain = NULL;
    }
};
             ...
             int           pooled_connection)
{
    ...
    if (!sock->is_ok()) {
        char_t buf[256];
        sock->get_error_text(buf, itemsof(buf));
...
int cli_fetch(int statement, int cursor_type)
{
    ...
    for (pb = stmt->params; pb != NULL; pb = pb->next) {
        ...
        if (pb->var_type == cli_asciiz) {
            msg_size += (int)((-msg_size & (sizeof(char_t)-1)) + (STRLEN((char_t*)pb->var_ptr) + 1)*sizeof(char_t));
        } else if (pb->var_type == cli_pasciiz) {
            msg_size += (int)((-msg_size & (sizeof(char_t)-1)) + (STRLEN(*(char_t**)pb->var_ptr) + 1)*sizeof(char_t));
        } else if (pb->var_type == cli_cstring) {
            msg_size += (int)(((cli_cstring_t*)pb->var_ptr)->len*sizeof(char_t) + 4);
        } else {
    ...
    if (!stmt->prepared) {
        *p++ = stmt->n_params;
        *p++ = stmt->n_columns;
        p = pack2(p, (int2)(stmt->stmt_len*sizeof(char_t) + stmt->n_params));
        pb = stmt->params;
    ...
    *p++ = cursor_type;
    for (pb = stmt->params; pb != NULL; pb = pb->next) {
        switch (pb->var_type) {
          case cli_asciiz:
            p += -(int)(size_t)p & (sizeof(char_t)-1);
...
static int cli_send_columns(int statement, int cmd)
{
    ...
    if (cmd == cli_cmd_update) {
        ...
        if (s->cursor_type != cli_cursor_for_update) {
    ...
    for (cb = s->columns; cb != NULL; cb = cb->next) {
        ...
        if (cb->var_type >= cli_array_of_oid && cb->var_type <= cli_array_of_string) {
            ...
            p = pack4(p, (int4)STRLEN((char_t*)src)+1);
            p = pack_str(p, (char_t*)src);
...
{
    return cli_insert(statement, oid);
}
    ...
    for (column_binding* cb = s->columns; cb != NULL; cb = cb->next) {  
        ...
        if (cb->set_fnc != NULL) {
            ...
            if (type == cli_array_of_string) { 
                char_t** s = (char_t**)dst;
#ifdef UNICODE
                char_t* q = (char_t*)((size_t)p & ~(sizeof(char_t)-1));
                while (--len >= 0) {
...
#ifdef UNICODE
                        ((cli_cstring_t*)dst)->ptr = (char_t*)((size_t)p & ~(sizeof(char_t)-1));
...
{
    int len = (int)(sizeof(cli_request) + STRLEN(table)*sizeof(char_t) + 1);
                            ...
                            cli_field_descriptor* columns)
{
    ...
    size_t size = sizeof(cli_request) + 4 + (STRLEN(tableName)+1)*sizeof(char_t);
...
int cli_drop_table(int session, char_t const* tableName)
{
    ...
    size_t size = sizeof(cli_request) + (STRLEN(tableName)+1)*sizeof(char_t);
    dbSmallBuffer<char> buf(size);
    cli_request* req = (cli_request*)buf.base();
    req->length  = (int4)size;
...
int cli_alter_index(int session, char_t const* tableName, char_t const* fieldName, int newFlags)
{
    ...
    size_t size = sizeof(cli_request) + 1 + (STRLEN(tableName)+STRLEN(fieldName)+2)*sizeof(char_t);
    dbSmallBuffer<char> buf(size);
    cli_request* req = (cli_request*)buf.base();
    req->length  = (int4)size;
