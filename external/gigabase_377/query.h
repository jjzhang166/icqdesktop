...
class GIGABASE_DLL_ENTRY dbQuery : public dbCompiledQuery {
  ...
  public:
    ...
    bool compileError() { return !isEmpty() && !compiled(); }
    ...
    bool isEmpty() { 
        return elements == NULL 
            || (elements->next == NULL && elements->type == dbQueryElement::qExpression && *(char_t*)elements->ptr == 0);
    }
...
inline dbQueryExpression& operator,(dbQueryExpression& expr, dbArray< dbReference<T> > const& value) { 
    return expr.add(dbQueryElement::qVarArrayOfRef, &value, &T::dbDescriptor);
}
...
template<class T>
inline dbQuery& operator,(dbQuery& query, dbArray< dbReference<T> > const& value) 
{ 
    return query.append(dbQueryElement::qVarArrayOfRef, &value, &T::dbDescriptor);
}
