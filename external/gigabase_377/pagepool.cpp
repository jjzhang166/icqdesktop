...
byte* dbPagePool::find(offs_t addr, int state)
{
    ...
    int hashCode = (int)((unsigned(addr) >> dbPageBits) & hashBits);
    int i, rc;
    mutex.lock();
    for (i = hashTable[hashCode]; i != 0; i = ph->collisionChain) {
        ...
        if (ph->offs == addr) {
                ...
                (state & dbPageHeader::psDirty))
            {
                ...
                ph->writeQueueIndex = (int4)(nDirtyPages++);
            } 
    ...
    if (i == 0) {
        ...
        unsigned h = (unsigned)((unsigned(ph->offs) >> dbPageBits) & hashBits);
        int* np;
...
bool dbPagePool::open(dbFile* file, offs_t fileSize)
{
    ...
    if (poolSize == 0) { // autodetect pool size
        ...
        if (totalMem << 1 >> 1 != totalMem) { 
            poolSize = (size_t)1 << (sizeof(totalMem)*8 - 2 - dbPageBits);
        } else { 
...
#if 0
        if (size_t(totalMem - poolSize*dbPageSize) > size_t(maxUnusedMemory)) {
...
{
    int i = (int)((size_t((byte*)ptr - buffer) >> dbPageBits) + 1);
    mutex.lock();
...
{
    size_t i = (size_t((byte*)ptr - buffer) >> dbPageBits) + 1;
    mutex.lock();
    dbPageHeader* ph = &pages[i];
    assert(ph->accessCount != 0);
    ph->accessCount += 1;
...
void dbPagePool::clear(offs_t fileSize)
{
    mutex.lock();
    assert(nDirtyPages == 0);
    pages->next = pages->prev = 0;
    this->fileSize = fileSize;
    for (int i = (int)poolSize+1; --i != 0;) {
        pages[i].state = 0;
        pages[i].next = i + 1;
    }
    pages[poolSize].next = 0;
    freePages = 1;
    memset(hashTable, 0, sizeof(int)*(hashBits+1));
    nPages = 0;
    mutex.unlock();
}
...
void dbPagePool::flush()
{
    ...
    if (nDirtyPages != 0) {
        ...
        for (int i = 0, n = (int)nDirtyPages; i < n; i++) {
...
void dbGetTie::set(dbPagePool& pool, offs_t pos, size_t size)
{
    reset();
...
void dbGetTie::set(dbPagePool& pool, offs_t pos)
{
	...
	assert(size <= GIGABASE_MAX_ROW_SIZE);
	if (size > GIGABASE_MAX_ROW_SIZE)
	{
		this->pool = NULL;
		this->obj = NULL;
		this->page = NULL;
...
void dbGetTie::fetch(dbPagePool& pool, offs_t pos)
{
    reset();
    int offs = (int)pos & (dbPageSize-1);
    byte* p = pool.get(pos - offs);
