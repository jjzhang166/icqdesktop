...
inline int map_type(dbFieldDescriptor* fd) { 
    return (fd->type < dbField::tpArray) 
        ? ((fd->indexType & DB_TIMESTAMP) ? cli_datetime 
           : ((fd->indexType & AUTOINCREMENT) ? cli_autoincrement : gb2cli_type_mapping[fd->type])) 
...
class GIGABASE_DLL_ENTRY dbCLI { 
  ...
  public:
    ...
    int xml_export(int session, FILE* out, char_t const* const* tables, int n_tables, cli_export_method method);
    ...
    int execute_query(int statement, int cursor_type, void* record_struct, va_list params);
    int execute_query(int statement, int cursor_type, void* record_struct, int n_params, int* param_types, void** param_values);
    ...
    int get_database_size(int session, cli_nat8_t* size);
