...
#define GB_TRY try { 
#define GB_CATCH } catch (dbException const&) { return cli_runtime_error; }
#else
#define GB_TRY 
#define GB_CATCH
#endif
                  ...
                  char const*   password)
{
    ...
    if (db == NULL) { 
        GB_TRY
        db = new dbDatabase((openAttr & cli_open_multiclient)
        ...
        if (openAttr & cli_open_do_not_reuse_oids_on_close) { 
            db->disableOidReuseOnClose();
        }
...
int dbCLI::create_statement(int session, char_t const* sql)
{
    ...
    stmt->params = NULL;
    stmt->session = s;
    stmt->cursor_type = cli_cursor_view_only;
...
int dbCLI::bind_parameter(int           statement,
                          ...
                          void*         var_ptr)
{
    if ((((unsigned)var_type >= cli_array_of_oid && var_type != cli_rectangle && var_type != cli_datetime) || var_type == cli_decimal)) 
    {
...
{
    return dbCLI::instance.fetch(statement, cursor_type);
}

int dbCLI::fetch(int statement, int cursor_type)
{
    ...
    stmt->cursor.reset();
    GB_TRY
    return stmt->cursor.select(stmt->query, (dbCursorType)cursor_type);
#ifdef THROW_EXCEPTION_ON_ERROR
...
{
    return dbCLI::instance.insert(statement, oid, false);
}

int cli_batch_insert(int statement, cli_oid_t* oid)
{
    return dbCLI::instance.insert(statement, oid, true);
}

int dbCLI::insert(int statement, cli_oid_t* oid, bool batch)
{
    statement_desc* stmt = statements.get(statement);    
    if (stmt == NULL) {
        return cli_bad_descriptor;
    ...
    dbAnyReference ref;
    GB_TRY
    if (!stmt->session->db->insertRecord(stmt->table, &ref, rec, batch)) { 
        stmt->oid = 0;
        return cli_not_unique;
    }
    GB_CATCH
    stmt->oid = ref.getOid();
    if (oid != NULL) { 
...
int dbCLI::update(int statement)
{
    ...
    if (stmt->cursor_type != cli_cursor_for_update) {
    ...
    if (stmt->record_struct == NULL) { 
        ...
        int rc = store_columns((char*)buf.base(), stmt, false);
        if (rc != cli_ok) { 
            stmt->cursor.setRecord(NULL);
            return rc;
        }
        succeed = stmt->cursor.update();
        stmt->cursor.setRecord(NULL);
    } else { 
        succeed = stmt->cursor.update();
    }
...
int cli_exec_batch(int session)
{
    return dbCLI::instance.exec_batch(session);
}

int dbCLI::exec_batch(int session)
{
    session_desc* s = sessions.get(session);
    if (s == NULL) {
        return cli_bad_descriptor;
        ...
        for (int i = 0; i < nColumns; i++, fp++) { 
            ...
            if (fd->bTree != 0) { 
                ...
                if (fp->type != cli_rectangle) { 
                    ...
                    if (tree->isThick()) { 
                        fp->flags |= cli_optimize_duplicates;
                    }
                    if (tree->isUnique()) { 
                        fp->flags |= cli_unique;
                    }
...
int dbCLI::describe_layout(int session, char_t const* table, cli_field_layout** fields, int* rec_size)
{
    session_desc* s = sessions.get(session);
    if (s == NULL) { 
        return cli_bad_descriptor;
        ...
        *rec_size = (int)desc->appSize;
        for (int i = 0; i < nColumns; i++, fp++) { 
            ...
            if (fd->bTree != 0) { 
                ...
                if (fp->desc.type != cli_rectangle) { 
                    dbGetTie tie;
                    dbBtree* tree = (dbBtree*)db->getRow(tie, fd->bTree);
                    if (tree->isCaseInsensitive()) { 
                        fp->desc.flags |= cli_case_insensitive;
                    }
                    if (tree->isThick()) { 
                        fp->desc.flags |= cli_optimize_duplicates;
                    }
                    if (tree->isUnique()) { 
                        fp->desc.flags |= cli_unique;
                    }
                }
            }
            if (fd->hashTable != 0) { 
...
{
    size_t varyingLength = (STRLEN(tableName) + 1)*sizeof(char_t);
    ...
    for (int i = 0; i < nColumns; i++, field += 1, offs -= sizeof(dbField)) {
        ...
        switch (type) {        
          ...
          case cli_rectangle:
            ...
            field->offset = (int4)DOALIGN(size, sizeof(cli_coord_t));
          ...
          case cli_pasciiz:
            ...
            field->offset = (int4)DOALIGN(size, sizeof(int4));
          ...
          case cli_array_of_string:
            ...
            SPRINTF(SPRINTF_BUFFER((char_t*)((char*)field + offs)), STRLITERAL("%s[]"), columns[i].name);
            offs += field->name.size*sizeof(char_t);    
            field->tableName.offs = offs;
        ...
        field->offset = (int4)DOALIGN(size, field->size);
...
int dbCLI::alter_index(dbDatabase* db, char_t const* tableName, char_t const* fieldName, int newFlags)
{
    ...
    if (fd->bTree != 0 && (newFlags & (cli_indexed|cli_hashed)) == 0) { 
        db->dropIndex(fd);
        fd->indexType &= ~(INDEXED|OPTIMIZE_DUPLICATES|CASE_INSENSITIVE|UNIQUE);
    } else if (fd->bTree == 0 && (newFlags & (cli_indexed|cli_hashed)) != 0) {
        fd->indexType |= newFlags & (cli_indexed|cli_hashed|cli_unique|cli_optimize_duplicates|cli_case_insensitive);
        db->createIndex(fd);
    }
    GB_CATCH
    return cli_ok;
...
int cli_execute_query(int statement, int cursor_type, void* record_struct, ...)
{
    ...
    int rc = dbCLI::instance.execute_query(statement, cursor_type, record_struct, args);
    va_end(args);
    return rc;
}

int dbCLI::execute_query(int statement, int cursor_type, void* record_struct, va_list params)
{
    statement_desc* stmt = statements.get(statement);    
    if (stmt == NULL || !stmt->prepared) {
        return cli_bad_descriptor;
    ...
    try { 
#endif
        return stmt->cursor.select(stmt->query, (dbCursorType)cursor_type, paramBase);
#ifdef THROW_EXCEPTION_ON_ERROR
    } catch (dbException const& x) { 
        return (x.getErrCode() == dbDatabase::QueryError)
            ? cli_bad_statement : cli_runtime_error;
...
{
    return dbCLI::instance.execute_query(statement, cursor_type, record_struct, n_params, param_types, param_values);
}

int dbCLI::execute_query(int statement, int cursor_type, void* record_struct, int n_params, int* param_types, void** param_values)
{
    statement_desc* stmt = statements.get(statement);    
    if (stmt == NULL || !stmt->prepared) {
        return cli_bad_descriptor;
    ...
    while (elem != NULL) { 
        if (elem->type != dbQueryElement::qExpression) { 
            ...
            switch (elem->type) { 
              ...
              case dbQueryElement::qVarStringPtr:
                offs = DOALIGN(offs, sizeof(char_t*));
                *(char_t**)(paramBase + offs) = *(char_t**)val;
                offs += sizeof(char_t*);
                break;
              case dbQueryElement::qVarReference:
                offs = DOALIGN(offs, sizeof(cli_oid_t));
...
int dbCLI::insert_struct(int session, char_t const* table_name, void* record_struct, cli_oid_t* oid)
{
    ...
    GB_TRY;
    dbAnyReference ref;
    if (!s->db->insertRecord(table, &ref, record_struct, false)) { 
        return cli_not_unique;
    }
    if (oid != NULL) { 
...
{
    return dbCLI::instance.xml_export(session, out, tables, n_tables, method);
}
...
int dbCLI::xml_export(int session, FILE* out, char_t const* const* tables, int n_tables, cli_export_method method)
{
    session_desc* s = sessions.get(session);
    if (s == NULL) { 
        return cli_bad_descriptor;
    ...
    GB_TRY
    s->db->exportDatabaseToXml(out, tables, n_tables, (dbDatabase::SelectionMethod)method);
    GB_CATCH
    return cli_ok;
}
...
int cli_get_database_size(int session, cli_nat8_t* size)
{
    return dbCLI::instance.get_database_size(session, size);
}

int dbCLI::get_database_size(int session, cli_nat8_t* size)
{    
    session_desc* s = sessions.get(session);
    if (s == NULL) { 
        return cli_bad_descriptor;
    }   
    *size = s->db->getDatabaseSize();
    return cli_ok;
}
