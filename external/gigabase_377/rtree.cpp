...
oid_t dbRtreePage::split_page(dbDatabase* db, branch const& br)
{
    ...
    dbPutTie tie;
    dbRtreePage* p = (dbRtreePage*)db->put(tie, pid);
...
{
    dbPutTie tie;
    dbRtreePage* pg = (dbRtreePage*)db->put(tie, pageId);
            ...
            if (b[i].rect & r) { 
                if (remove(db, r, b[i].p, recordId, level, rlist)) { 
                    ...
                    if (p->n >= min_fill) { 
                        ...
                        db->pool.unfix(p);
                        p = (dbRtreePage*)db->put(tie, b[i].p);
...
void dbRtreeIterator::init(dbDatabase* db, oid_t treeId, dbSearchContext& sc) 
{
    this->db = db;
    this->sc = sc;            
    this->treeId = treeId;
...
oid_t dbRtreeIterator::first()
{
    dbGetTie tie;
    dbRtree* tree = (dbRtree*)db->getRow(tie, treeId);
    height = tree->height;

    if (height == 0) { 
        return 0;
    }
    return gotoFirstItem(0, tree->root);
}

oid_t dbRtreeIterator::last()
{
    dbGetTie tie;
    dbRtree* tree = (dbRtree*)db->getRow(tie, treeId);
    height = tree->height;
    ...
    return gotoLastItem(0, tree->root);
}


oid_t dbRtreeIterator::next()
{
    rectangle& r = *(rectangle*)sc.firstKey;
    int sp = height;
    while (--sp >= 0) { 
        dbRtreePage* pg = (dbRtreePage*)db->get(pageStack[sp]);
        for (int i = posStack[sp], n = pg->n; ++i < n;) { 
            if (r & pg->b[i].rect) { 
                oid_t curr = pg->b[i].p;
                if (sp+1 == height) { 
                    if (sc.condition != NULL && !db->evaluateBoolean(sc.condition, curr, sc.cursor->table, sc.cursor)) { 
                        continue;
                    }
                } else if ((curr = gotoFirstItem(sp+1, curr)) == 0) { 
                    continue;
                }
                pageStack[sp] = pageStack[sp];
                posStack[sp] = i;
                db->pool.unfix(pg);
                return curr;
            }
...
oid_t dbRtreeIterator::prev()
{
    rectangle& r = *(rectangle*)sc.firstKey;
    int sp = height;
    while (--sp >= 0) { 
        dbRtreePage* pg = (dbRtreePage*)db->get(pageStack[sp]);
        for (int i = posStack[sp]; --i >= 0;) { 
            if (r & pg->b[i].rect) { 
                oid_t curr = pg->b[i].p;
                if (sp+1 == height) { 
                    if (sc.condition != NULL && !db->evaluateBoolean(sc.condition, curr, sc.cursor->table, sc.cursor)) { 
                ...
                } else if ((curr = gotoLastItem(sp+1, curr)) == 0) { 
                    continue;
                }
                pageStack[sp] = pageStack[sp];
                posStack[sp] = i;
...
oid_t dbRtreeIterator::gotoFirstItem(int sp, oid_t pageId) 
{ 
    dbRtreePage* pg = (dbRtreePage*)db->get(pageId);
    rectangle& r = *(rectangle*)sc.firstKey;
...
oid_t dbRtreeIterator::gotoLastItem(int sp, oid_t pageId) 
{ 
    dbRtreePage* pg = (dbRtreePage*)db->get(pageId);
    rectangle& r = *(rectangle*)sc.firstKey;
