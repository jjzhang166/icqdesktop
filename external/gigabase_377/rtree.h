...
class GIGABASE_DLL_ENTRY dbRtreeIterator : public dbAbstractIterator { 
  public:
    void init(dbDatabase* db, oid_t treeId, dbSearchContext& sc);
  ...
  private:
    oid_t gotoLastItem(int sp, oid_t pageId);
    oid_t gotoFirstItem(int sp, oid_t pageId);
    ...
    oid_t           treeId;
    dbSearchContext sc;
    oid_t           pageStack[MAX_RTREE_HEIGHT];
    int             posStack[MAX_RTREE_HEIGHT];
};
