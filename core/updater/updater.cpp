...
namespace core
{
	namespace update
	{
		...
		int32_t run(const update_params& _params);
		...
		int32_t run(const update_params& _params)
		{
			http_request_simple request(_params.is_stoped_);
			...
			if (local_version_info < server_version_info)
				return do_update(iter_file->value.GetString(), iter_md5->value.GetString(), _params);
