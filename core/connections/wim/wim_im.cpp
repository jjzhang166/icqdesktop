...
const core::wim::wim_packet_params core::wim::im::make_wim_params()
{
	...
	return wim_packet_params(
		stop_handler,
		auth_params_->a_token_,
		auth_params_->session_key_,
		auth_params_->dev_id_,
		auth_params_->aimsid_,
...
void core::wim::im::login(const login_info& _info)
{
	switch (_info.get_login_type())
	{
		case login_type::lt_login_password:
			{
				login_by_password(_info.get_login(), _info.get_password(), _info.get_save_auth_data());
			}
			break;
		default:
			{
...
void core::wim::im::login_by_password(const std::string& login, const std::string& password, bool save_auth_data)
{
	auto packet = std::make_shared<client_login>(make_wim_params(), login, password);

	std::weak_ptr<wim::im> wr_this = shared_from_this();

	post_wim_packet(packet)->on_result_ = [wr_this, packet, save_auth_data](int32_t _error)
	{
		...
		if (_error == 0)
		{
			...
			ptr_this->on_login_result(0);
			ptr_this->start_session();
		}
		else
		{
			ptr_this->on_login_result(_error);
		}
	};
}
...
void im::get_sticker(int64_t _seq, int32_t _set_id, int32_t _sticker_id, core::sticker_size _size)
{
        ...
        [_seq, _set_id, _sticker_id, _size, wr_this](tools::binary_stream& _sticker_data)
    {
        ...
        if (!ptr_this->get_stickers()->is_download_in_progress())
        {
...
void core::wim::im::on_login_result(int32_t err)
{
    ...
    g_core->post_message_to_gui("login_result", 0, cl_coll.get());
}
...
void core::wim::im::on_event_typing(fetch_event_typing *_event, std::shared_ptr<auto_callback> _on_complete)
{
    ...
    if (_event->chatters_aim_ids().size())
    {
        ifptr<iarray> array(coll->create_array());
        array->reserve((int32_t)_event->chatters_aim_ids().size());
        for (auto c: _event->chatters_aim_ids())
        {
        ...
        coll.set_value_as_array("chattersAimIds", array.get());
    }
...
void im::get_archive_messages(int64_t _seq, const std::string& _contact, int64_t _from, int64_t _count, int32_t _recursion)
{
	...
	bool first_request = (_from == -1);

	// load from local storage
	get_archive()->get_messages(_contact, _from, _count)->on_result = [_seq, wr_this, _contact, _recursion, first_request](std::shared_ptr<archive::history_block> _messages)
	{
		auto ptr_this = wr_this.lock();
		if (!ptr_this)
			return;

		ptr_this->get_archive()->get_dlg_state(_contact)->on_result = [_seq, wr_this, _contact, _recursion, first_request, _messages](const archive::dlg_state& _state)
		{
			auto ptr_this = wr_this.lock();
			if (!ptr_this)
				return;
			...
			auto as = std::make_shared<tools::auto_scope>([coll, _seq]()
			{
			...
			if (first_request)
			{
				...
				ptr_this->get_archive()->get_not_sent_messages(_contact)->on_result = [_seq, wr_this, _contact, as, coll](std::shared_ptr<archive::history_block> _pending_messages)
				{
					auto ptr_this = wr_this.lock();
					if (!ptr_this)
						return;

					if (_pending_messages->size())
						serialize_messages_4_gui("pending_messages", _pending_messages, coll.get(), ptr_this->auth_params_->time_offset_);
				};
			}
			...
			ptr_this->get_archive()->set_dlg_state(contact, *(packet->get_dlg_state()))->on_result = [wr_this, contact, packet, on_result]()
			{
				...
				if (!messages->empty())
				{
					remove_messages_from_not_sent(ptr_this->get_archive(), contact, messages)->on_result_ = [wr_this, contact, messages, on_result, state](int32_t _error)
					{
                        ...
                        if (ptr_this->favorites_->contains(contact))
                            ptr_this->post_dlg_state_to_gui(contact);

						ptr_this->get_archive()->update_history(contact, messages)->on_result =	[wr_this, contact, on_result](std::shared_ptr<archive::headers_list> _inserted_messages)
						{
							on_result(0);
						};
					};
...
void im::on_event_hidden_chat(fetch_event_hidden_chat* _event, std::shared_ptr<auto_callback> _on_complete)
{
	...
	active_dialogs_->remove(_event->get_aimid());

	coll_helper cl_coll(g_core->create_collection(), true);
	cl_coll.set_value_as_string("contact", _event->get_aimid());
	g_core->post_message_to_gui("active_dialogs_hide", 0, cl_coll.get());
...
void core::wim::im::login_normalize_phone(int64_t _seq, const std::string& _country, const std::string& _raw_phone, const std::string& _locale)
{
	...
	post_wim_packet(packet)->on_result_ = [packet, _seq, wr_this, _locale](int32_t _error)
	{
		std::shared_ptr<wim::im> ptr_this = wr_this.lock();
		if (!ptr_this)
			return;
		...
		if (0 == _error)
		{
			...
			ptr_this->login_get_sms_code(_seq, _info);
		}
		else
		{
...
void core::wim::im::login_get_sms_code(int64_t _seq, const phone_info& _info)
{
	...
	post_wim_packet(packet)->on_result_ = [packet, _seq, wr_this](int32_t _error)
	{
		...
		if (_error == 0)
		{
			...
			ptr_this->phone_registration_data_ = std::make_shared<phone_info>();
			ptr_this->phone_registration_data_->set_phone(packet->get_phone());
			ptr_this->phone_registration_data_->set_trans_id(packet->get_trans_id());
			ptr_this->phone_registration_data_->set_existing(packet->get_existing());
