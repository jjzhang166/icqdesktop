...
void core::im_container::on_voip_call_message(int64_t _seq, coll_helper& _params)
{
	...
	if (type == "voip_call_start") {
		...
		im->on_voip_call_start(_params.get_value_as_string("contact"), call_type == "video", mode == "attach");
		...
		const std::string mode = _params.get_value_as_string("mode");
		im->on_voip_call_accept(_params.get_value_as_string("contact"), mode == "video");
...
void core::im_container::on_login_get_sms_code(int64_t _seq, coll_helper& _params)
{
	...
	ims_.clear();
	auto im = std::make_shared<wim::im>(im_login_id(""), voip_manager_);
	im->login_normalize_phone(_seq, _params.get_value_as_string("country"), _params.get_value_as_string("phone"), _params.get_value_as_string("locale"));
	ims_.push_back(im);
}
...
void core::im_container::unlogin()
{
    assert(!ims_.empty());
    if (ims_.empty())
        return;
